...
public:
    ...
    void delete_front()
    {
        ...
        // Integrity change
        if (head)
            head->prev = nullptr; // the ll vs dll
        else if (!length)
            tail = nullptr;
    ...
    void delete_all_nodes_with_key(int value)
    {
        ...
        while (cur)
        {
            if (cur->data == value)
            {
                Node *temp = cur->next; // Store the next node before deleting current
                delete_node(cur);       // Delete the current node
        ...
        debug_verify_data_integrity(); // Verify data integrity after deletion
    }
    ...
    bool is_palindrome()
    {
        ...
        while (left != right && left->prev != right)//left becomes equal to right (in case of an odd number of nodes) or
        // left->prev becomes equal to right (in case of an even number of nodes).This ensures that both pointers meet at the middle of the list or cross each other,
        // indicating that all corresponding elements have been compared for equality.
        {
